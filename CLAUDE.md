...
| v2 | Gemini による校正（P-2 の解決） | **実装中**（校正と料金ログ・表示まで完了。課金履歴画面・価格設定UI・上限ガード・実画面検証が残る） |
...
設定画面からの単価編集はまだないため、現時点では `pricing.json` を直接編集する。
...
ログ書き込みや集計表示の失敗は、既に受け取った校正結果や既存のエラー処理を妨げない。履歴画面、月間上限ガード、
...
次の WIP は **課金履歴画面（期間・種別フィルター、明細表示）**。
その後に、`pricing.json` の設定画面編集、月額上限の進捗・ガード、CSV エクスポートと古い明細の圧縮、
v3 の few-shot 選定・スタイルガイド生成へ進む。

注意点:

- `api_calls` はUSD/JPYログと直近・起動後・当日・当月の常時表示を実装済み。履歴画面・月額上限・CSV/圧縮はまだない。
