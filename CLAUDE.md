...
| v2 | Gemini による校正（P-2 の解決） | **実装完了**（実機確認待ちがトレイアイコンの4状態表示のみ） |
...
## トレイアイコンの4状態表示（2026-07-31 実装）

requirements.md §3.1.1 の最後の未実装項目。
...
（同日あとから実装した CSV エクスポートと明細圧縮も同日中に確認済み。
未確認で残っているのはトレイアイコンの4状態表示だけ。下の「未確認のまま残っていること」参照）。
...
- **トレイアイコンの4状態表示（2026-07-31 実装分）が丸ごと未確認。** 見てもらいたいのは、
  (1) 通知領域の実サイズ（16px、高DPIなら20/24px）で4状態が区別できるか、
  (2) 校正の実行中だけ「校正中」になり、終わると元へ戻るか、
  (3) API 失敗後にエラー表示が残り、次の成功で消えるか、
  (4) 月間上限に到達した状態で上限アイコンになり、その最中に手動校正すると一時的に校正中へ変わるか、
  (5) ツールチップの先頭に `[校正中]` などが出るか、(6) ライト/ダークのタスクバーで見え方が破綻しないか。
  (3) は実 API を呼ばずに再現しにくい（キーを一時的に不正な値にすれば 400 で確認できる。
  失敗するだけなので課金は発生しない見込みだが、実行前に必ず確認を取ること）。
...
(b) CSV エクスポートと明細圧縮、(c) トレイアイコンの4状態表示は、いずれも 2026-07-31 に完了した。
v2 の実装は全項目が済んでいる。

ただし **(c) の実機確認だけが未実施**（この環境では画面キャプチャが使えないため、
4状態のアイコンが通知領域で実際に区別できるかはユーザーに見てもらうしかない）。
実装のブロッカーではないので v3 の着手は待たせないが、v2 を「完了」と書く前に確認が要る。
確認したい点は上の「未確認のまま残っていること」参照。
