...
直近の確認では `PromptValidation --self-test` が全件合格し、本体ビルドは警告0・エラー0。
自己テストはHTTPスタブと一時SQLiteだけを使い、実Gemini APIは呼んでいない。
本体にはまだ校正提案を簡単に注入する開発用経路がないため、実画面での一連の操作と実API疎通は未確認。

次に行うこと:

1. `pricing.json` の読込とモデル別単価計算を実装し、`MainWindow` に残る単価ハードコードを除く。
2. `api_calls` へ成功・失敗・所要時間・トークン・料金・提案/破棄件数を記録する。
3. 直近・セッション・当日・当月の料金をステータスバーへ常時表示する。
4. Frankfurterの日次為替キャッシュと円換算を追加する。
5. 課金履歴画面、月間上限ガード、CSVエクスポートへ進む。
6. v2完了後に、リアクションのfew-shot選定からv3の学習機能へ進む。
