...
「次点」の判断材料には含めない。モデル単価セクションのコンボ切替時の生テキスト保持だけは
`pricing.json` にモデルが1件しか無く現状では検証不能なため未確認のまま残っているが、これは
実装のブロッカーではないので次点の判断には影響しない）。(a) は2026-07-31に完了した。(a)(b) は元々この順で計画していた
