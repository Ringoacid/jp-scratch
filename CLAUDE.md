...
- 起動後にバックグラウンド（`App.xaml.cs` の `CompactApiLogs`）で走らせる。起動経路の同期処理に足すと
  コールドスタートの実測値（0.63秒）を落としかねない。
- **実機確認は未実施**（CSVの実ファイル書き出しと Excel での文字化け有無、設定画面「明細の保持期間」欄の
  見た目、圧縮後の課金履歴画面の表示）。
...
- **課金履歴のCSVエクスポートと、保持期限後の明細圧縮**（`Services/BillingCsvExporter.cs`、
  ...
  設定画面「明細の保持期間」欄。詳細は上の専用セクション）。**実機確認は未実施**
...
- CSVエクスポートと明細圧縮の実機確認一式。具体的には (a) 「CSV出力」ボタンで実際にファイルが
  書き出され、Excel で開いたときに日本語が化けないこと、(b) 設定画面「明細の保持期間」欄の見た目、
  (c) 保持期間を短くして圧縮を走らせたあと、課金履歴画面のヘッダに
  「うち N 件は保持期限を過ぎ日次サマリへ圧縮済み（明細なし）」が出て、期間合計が圧縮前と
  一致すること。(c) は `--seed-billing` の隔離データディレクトリで確認できる（実APIは不要）。
