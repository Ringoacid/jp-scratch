...
| v2 | Gemini による校正（P-2 の解決） | **実装中**（校正・料金ログ・表示・課金履歴画面・月額上限ガード・`pricing.json` 設定画面編集・CSVエクスポート・明細圧縮まで完了。トレイアイコンの4状態表示が残る） |
...
- **圧縮後の課金履歴画面の実機確認は未実施**（CSVの書き出しと Excel での文字化け無しは
  2026-07-31 に確認済み）。
...
- **課金履歴のCSVエクスポートと、保持期限後の明細圧縮**（`Services/BillingCsvExporter.cs`、
  ...
  設定画面「明細の保持期間」欄。詳細は上の専用セクション）。CSV の書き出しと Excel での
  文字化け無しは 2026-07-31 に実機確認済み
...
- 設定画面「モデル単価」セクションの実機確認（2026-07-31、隔離データディレクトリ
  ...
  ことも確認。ただし `pricing.json` 登録モデルが `gemini-3.5-flash-lite` の1件のみのため、コンボでの
  モデル切替時に生テキストが保持されるかは確認できていない（詳細は下の「未確認のまま残っていること」）。
...
FAIL 0、exit code 0**（本体ビルドも警告0・エラー0）。
...
**2026-07-31 に、月間上限と課金履歴画面の実機確認負債はすべて解消した**
（同日あとから実装した CSV エクスポートと明細圧縮の分は未確認。下の「未確認のまま残っていること」参照）。
...
- 明細圧縮の実機確認: 保持期間を短くして圧縮を走らせたあと、課金履歴画面のヘッダに
  「うち N 件は保持期限を過ぎ日次サマリへ圧縮済み（明細なし）」が出て、期間合計が圧縮前と
  一致すること。`--seed-billing` の隔離データディレクトリで確認できる（実APIは不要）。
  シードは4か月前に3件・2か月前に2件を置くので、保持期間を **1**（5件すべて）または
  **3**（古い方の3件だけ）にすれば踏める。既定の12では圧縮されない。
  設定画面「明細の保持期間」欄の見た目（補足文が切れていないこと）も同時に確認する。
  なお CSV の書き出しと Excel での文字化け無しは 2026-07-31 に確認済み。
- 設定画面「モデル単価」セクションで、`pricing.json` にモデルが1件（`gemini-3.5-flash-lite`）しか
  登録されていないため、コンボでのモデル切替時に入力途中の生テキストが保持されるかだけ未確認
  （それ以外の見た目・不正入力時のエラー表示とフォーカス移動・保存内容の反映は2026-07-31に確認済み）。
- この環境では画面キャプチャが使えない（`CopyFromScreen` が失敗する）ため、見た目・実キー入力・
  IME 挙動は自動検証できない。実機確認は引き続きユーザーへ依頼する。

## 次の WIP と、その判断理由

次の WIP は **トレイアイコンの4状態表示**（requirements.md 3.1.1、通常/校正中/APIエラー/月間上限到達）。

理由: v2 チェックリストの残りは (a) `pricing.json` 設定画面編集、(b) CSV エクスポートと明細圧縮、
(c) トレイアイコンの4状態表示、の3つだった。(a)(b) はいずれも 2026-07-31 に完了したので、
残るのは (c) だけになった。(c) は月間上限の実装中に「未実装だった」と判明したもので、
通知（バルーン）とアイコンの出し分けは別物であり、影響範囲が 3.1.1 という別要件番号に閉じるため、
月間上限のWIPには含めず独立WIPとして切り出してある。
(c) を終えたら v3（few-shot 選定・スタイルガイド生成）へ進む。

CSV エクスポートと明細圧縮の**実機確認は未実施**だが、これは実装のブロッカーではないので
(c) の着手を待たせない（同様に、モデル単価セクションのコンボ切替時の生テキスト保持も
`pricing.json` にモデルが1件しか無く現状では検証不能なまま残っている）。
