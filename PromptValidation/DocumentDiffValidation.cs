...
internal static class DocumentDiffValidation
{
    ...
    internal static bool RunSelfTests()
    {
        ...
        [
            ...
            // 文頭への挿入と、その直後への挿入。間に挟まる書記素が語要素でない（読点）ため
            // MergeNearbyWordHunks で連結されず、2つの独立した挿入として残る。
            // 純粋な挿入は TextAnchor で監視できるよう隣接する1書記素を借りて置換に変換するが、
            // 借りる方向を調停しないと両方が同じ書記素（先頭の読点）を奪い合って範囲が重複し、
            // 「変更範囲が重複しています」として正しい校正結果が丸ごと破棄され課金だけが残る。
            ("文頭挿入＋直後挿入", "、文章です", "あ、い文章です", 2, true),
