...
全文をコピーしながら消えます。ふだんはタスクトレイに常駐しているだけなので、じゃまになりません。

<img src="docs/images/main-light.png" alt="JP Scratch のメインウィンドウ。上部にタブが3つ並び、本文と行番号、下部にステータスバーが表示されている。" width="620">
...
<img src="docs/images/main-dark.png" alt="ダークテーマのメインウィンドウ。タブが3つ並んでいる。" width="620">
...
AI の利用料は**使った分だけ**かかります。JP Scratch はそれを隠しません。

- ウィンドウ下端に、今月いくら使ったかが常に出ています。
- **月間の上限額**を決めておくと、そこに達した時点で自動校正が止まります。
- 1 回ごとの明細（いつ・どのモデル・何トークン・いくら）は課金履歴画面で確認できます。CSV にも出せます。

<img src="docs/images/billing-history.png" alt="課金履歴ウィンドウ。期間と種別で絞り込むフィルタ、当月の集計、日時・種別・モデル・入出力トークン・USD・JPY・所要時間・成否・提案数の一覧表。" width="1240">
...
<img src="docs/images/settings-proofreading.png" alt="設定画面の「校正」タブ。自動校正のモデルと手動校正のモデルをそれぞれ選ぶドロップダウン、タイムアウト、月間上限額などの設定項目。" width="760">
...
全 11 モデルを実測した結果は下の [校正モデルの比較](#校正モデルの比較実測) にあります。ざっくりまとめると:
...
- 校正対象は空行区切りの段落（空行がなければ 1 行単位）で決め、**変更された段落だけ**を送る。
...
11 モデルすべてに同じ日本語文章 7 本を 3 回ずつ投げて実測した（2026-08-06、231 リクエスト、実費 $2.05）。
詳細は [`PromptValidation/model-benchmark-2026-08-06.md`](PromptValidation/model-benchmark-2026-08-06.md)、
...
<picture>
  ...
  <img src="docs/images/model-benchmark-scatter-light.png" alt="校正1回の所要時間と料金の散布図。両軸とも対数目盛。Claude Haiku 4.5、Gemini 3.5 Flash Lite、GPT 5.6 Luna が速さと安さの両方で他に負けていない。">
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/images/model-benchmark-bars-dark.png">
  <img src="docs/images/model-benchmark-bars-light.png" alt="モデル別の所要時間（中央値と最小〜最大）と1回あたりの提案件数。行はプロバイダーごとにまとまっている。">
</picture>
...
`pricing.json` の既定値もそこから生成する。設定画面の「モデル単価」欄からモデルごとに変更できる。
