# JP Scratch

WSL や一部アプリで日本語が打てないときに、**どこからでも 1 アクションで呼び出せてすぐ消える**常駐メモ帳。

仕様は [requirements.md](requirements.md) を参照。現在 **v1（常駐エディタ）と v2（Gemini / GPT-5.6 Luna による校正）は実装済み**。
v2はAPIキー管理、Gemini / OpenAIクライアント、段落単位の送信、自動校正、全文差分と編集追従、
インライン波線と提案パネル、許可・拒否・理由つき別案、リアクション保存に加えて、
モデル単価の設定画面編集、課金履歴画面、月間上限の進捗表示とガード、CSVエクスポート、
明細の保持期限後圧縮、トレイアイコンの4状態表示まで完了している。
直近・起動後・当日・当月のUSD/JPY利用額は、編集統計・通知とは別のステータスバー下段に常時表示する。
次の WIP は v3（文体の学習）で、こちらは未着手。

---

## 使い方

| 操作 | 割り当て |
|---|---|
| 表示 / 非表示 | `Alt + Space`（グローバル） |
| 全文をコピーして隠す | `Ctrl + Alt + Enter`（グローバル。直前のウィンドウにフォーカスを戻す） |
| 新しいタブ / 閉じる / 復元 | `Ctrl+T` / `Ctrl+W` / `Ctrl+Shift+T` |
| タブ切替 | `Ctrl+Tab` / `Ctrl+Shift+Tab` |
| 検索 / 置換 | `Ctrl+F` / `Ctrl+H`、次・前は `F3` / `Shift+F3` |
| 全タブ横断検索 | `Ctrl+Shift+F` |
| `.txt` へエクスポート | `Ctrl+Shift+S` |
| 変更段落または選択範囲を校正 | `Ctrl+Enter` |
| 次 / 前の校正提案 | `F8` / `Shift+F8` |
| 校正提案を許可 / 拒否 | `Ctrl+.` / `Ctrl+,` |
| 隠す | `Esc` |
| フォントサイズ | `Ctrl + マウスホイール` |

- タブ見出しは**ダブルクリックでリネーム**、**中クリックで閉じる**、**ドラッグで並べ替え**。
- 設定の「隠すときに本文をクリップボードへコピーする」を ON にすると、**どの隠し方でも**コピーされる
  （フォーカス喪失・`Alt+Space`・`Esc`・`✕`）。
- 「保存」操作はない。入力が止まると自動で書き込まれる。
- 終了はトレイアイコンの右クリック →「終了」。ウィンドウの `✕` は隠すだけ。

## データの置き場所

```
%APPDATA%\JpScratch\
├─ settings.json      設定
├─ pricing.json       モデル別のAPI単価と更新日
├─ credentials.dat   DPAPIで暗号化したGemini / OpenAI APIキー（アプリ保存時のみ）
├─ app.db             タブのメタ情報・API課金ログ・校正リアクション・為替/取得試行メタデータ（SQLite v3）
└─ tabs\
   ├─ {id}.txt        本文（UTF-8, BOM なし）
   └─ trash\{id}.txt  閉じたタブ（既定 30 日で自動削除）
```

本文は**プレーンテキスト**で保存する。このアプリが動かなくなってもメモ帳でサルベージできることを要件にしているため。
書き込みは一時ファイルに保存してから `File.Replace` で置き換えるため、書き込み中に落ちても既存の本文は壊れない。

---

## ビルド

前提: .NET 10 SDK

```powershell
dotnet build                 # デバッグビルド
dotnet run                   # 実行
```

### 煙テスト

```powershell
powershell -File tools\smoke-test.ps1 publish\fdd\JpScratch.exe
```

起動・日本語入力・自動保存・BOM なし UTF-8・非表示時のメモリ返却・二重起動時の呼び戻し・
再表示後のキャレット位置までを通しで確認する。UI 自動化は使わず `WM_CHAR` の送出だけで動く。

> **`%APPDATA%\JpScratch` を消してから走る**ので、実データがある状態では実行しないこと。

### 校正APIキーとモデル

本体の設定画面にある「校正」から使用モデルを選び、Gemini または OpenAI の APIキーを登録できる。登録したキーはWindowsの
DPAPI（現在のユーザー）で暗号化し、`%APPDATA%\JpScratch\credentials.dat` に保存する。
既存のキーは画面へ再表示せず、保存の有無だけを示す。

選択できるモデルは `gemini-3.5-flash-lite` と `gpt-5.6-luna`。Gemini は `GEMINI_API_KEY`、
OpenAI は `OPENAI_API_KEY` を環境変数から取得できる。対象の環境変数が見つかった場合は、初回検出時に
環境変数とアプリ保存キーのどちらを使うか確認し、選択を記憶する。取得元の変更と保存済みキーの削除は設定画面から行える。

既定のモデル単価は Gemini が入力 $0.30 / 出力 $2.50、GPT-5.6 Luna が入力 $0.20 /
出力 $1.20（いずれも 1M tokens あたり）。「モデル単価」欄からモデルごとに変更でき、
既に記録済みの課金ログは再計算しない。

校正対象は空行区切りの段落（空行がなければ1行単位）で決め、変更された段落だけを送る。
前後1段落は修正対象外の文脈として添え、2,000文字を超える対象は書記素境界を保って分割する。
同じ内容を再送しないための比較には、前回送信済みスナップショットのSHA-256ハッシュを使う。
校正提案は本文中の波線と下部パネルに表示し、下線クリックまたはパネルの矢印で個別に選択できる。
提案がなければパネルは畳まれ、エディタ領域を狭めない。
選択した提案は許可・拒否でき、理由つき拒否では過去によく使った理由を候補表示する。
リアクションはSQLiteへ残るため、許可後に `Ctrl+Z` で本文を戻しても記録は消えない。
理由つき別案生成は、課金確認がONなら料金単価を示す確認の後にAPIを呼び、OFFなら確認なしで呼び出す。
いずれの場合も使用トークン数と料金を応答後に表示する。
設定画面の「課金API実行前の確認を表示する」をOFFにすると、自動校正・手動校正・理由付き別案生成の
いずれでも実行前確認を表示しない。単価は初回起動時に `pricing.json` へ生成し、モデル別に読み込む。設定画面の「モデル単価」から編集でき、
既に記録済みのログは再計算しない（単価の変更は以後の呼び出しにだけ効く）。料金は内部で `decimal` として計算し、
表示はUSDが小数点以下最大8桁、JPYが通常小数第2位まで（微小額は第3位まで）である。
API送信ごとに、成否・所要時間・入力トークン・出力/推論トークン・USD料金・提案/破棄件数を
`app.db` の `api_calls` へ保存する。出力トークンは `candidatesTokenCount + thoughtsTokenCount` の
課金対象値である。Frankfurter v2（ECB）のUSD/JPYを
`https://api.frankfurter.dev/v2/rate/USD/JPY?providers=ECB` から日次キャッシュし、各ログにはその時点の
レート・基準日・円額を固定保存する。最後に記録したリクエストにはその基準日を併記し、起動後・当日・当月の
USD/JPY累計は保存済みの固定レートだけで集計する。複数レート時はツールチップへ日付範囲・件数を示す。
過去の円額が欠ける期間は安全に
`¥—` と表示する。`Ctrl+Shift+B`（トレイメニュー・ステータスバーのクリックでも可）で開く課金履歴画面から、
期間・トリガー種別で絞り込んだ明細と集計を確認し、CSVへ出力できる。月間上限に達すると自動校正を止め、
トレイのアイコンと通知、ステータスバーの進捗バーで知らせる（手動実行は確認のうえ続行できる）。
保持期間を過ぎた明細は日次サマリへ圧縮する（期間合計は変わらない）。
為替取得は成功・失敗を問わずローカル日付ごとに最大1回だけ試行し、失敗時は古いキャッシュを使って次の日まで再試行しない。
複数段落の校正途中で失敗しても、成功済み応答と使用量が分かる失敗応答の固定スナップショットを合算して表示する。
`F8` / `Shift+F8` で提案を移動し、`Ctrl+.` / `Ctrl+,` で許可・拒否できる。

自動校正は入力停止後2秒で発火し、API送信の間を最低10秒空ける。どちらの値も設定画面で
変更でき、自動校正だけをOFFにして `Ctrl+Enter` の手動校正だけを使うこともできる。
「課金API実行前の確認を表示する」をOFFにすると、自動校正・手動校正・理由付き別案生成の実行前確認を
まとめて省略できる。校正中の状態はステータスバーとトレイアイコンで示し、進捗メッセージの表示/間隔を
設定する項目は持たない。
送信済み段落のハッシュが変わっていなければ再送しない。選択範囲がある状態の手動校正では
その範囲だけを対象にする。通信待ちの間に本文やタブが変わった場合は、残りの送信と結果反映を中止する。

### 校正機能の検証

校正プロンプトの比較に使用し、現在は本体と実装を共有して回帰テストも行う検証アプリが
`PromptValidation/` にある。

```powershell
$env:GEMINI_API_KEY = "..."
dotnet run --project PromptValidation
```

誤り例5本と文体保護例25本を一括評価する。個別実行、任意文章、ドライランなどの詳細は
[`PromptValidation/README.md`](PromptValidation/README.md) を参照。

### トレイアイコンの作り直し

トレイは 4 状態（通常 / 校正中 / API エラー / 月間上限到達）をアイコンで示す。

- **`Assets/app.ico` は手作りで、自動生成物ではない。** 作り直すときは 16 / 20 / 24 / 32 / 40 / 48 / 64 / 256 px を
  含め、**256px だけを PNG 圧縮、それ以外は DIB（無圧縮）で格納**すること。`System.Drawing.Icon`
  （通知領域の `NotifyIcon` が使う）は PNG 圧縮エントリを展開できず、小サイズを PNG にすると
  トレイのアイコンが黙って既定のアイコンへ差し替わる。
- 状態アイコン（`app-proofreading.ico` / `app-error.ico` / `app-limit.ico`）は `app.ico` の派生物で、
  各サイズの絵をそのまま土台にして右下へバッジを重ねたもの。次のコマンドで生成し直せる
  （要 Python 3 と Pillow。ビルドや実行には不要）。

```bash
python tools/build-tray-icons.py
```

`--check` を付けると書き換えず、リポジトリ内のファイルが生成結果と一致するかだけを確認する。
格納形式の検査は `PromptValidation --self-test` にも入っている（4 ファイルが `app.ico` と
同じサイズ構成で、256px 以外が DIB であること）。

### インストーラー (MSI)

前提: WiX v5（`dotnet tool install --global wix --version 5.0.2`）

```powershell
powershell -File installer\build.ps1                  # フレームワーク依存 (約 1.8 MB)
powershell -File installer\build.ps1 -SelfContained   # ランタイム同梱 (約 70 MB)
```

出力は `publish\msi\`。ユーザー単位インストール（`%LOCALAPPDATA%\Programs\JP Scratch`）なので管理者権限は不要。
スタートメニューのショートカットと `HKCU\...\Run` のスタートアップ登録を作る。

> WiX v6 以降は Open Source Maintenance Fee の EULA 同意が必要になる。
> 同意するかは利用者側の判断なので、ここでは v5 に固定している。

---

## 構成

```
App.xaml.cs              起動・常駐・単一インスタンス・クラッシュ時の保存
Assets/app.ico           アプリアイコン（小サイズは DIB。NotifyIcon が PNG を展開できないため）
Assets/app-*.ico         トレイの状態アイコン（校正中 / APIエラー / 月間上限到達）
Controls/                検索・置換パネル
Editor/                  AvalonEdit の拡張（検索、不可視文字、フォント、校正提案の描画）
Infrastructure/          Win32 相互運用、一時ファイル経由の安全なファイル書き込み、パス解決
Models/                  設定・タブ・ホットキー
Proofreading/             Gemini / OpenAIクライアント、プロンプト、段落計画、差分、提案セッション
Services/                設定・SQLite・資格情報・リアクション・タブ・ホットキー・配置・テーマ・トレイ
Themes/                  ライト / ダーク / 共通スタイル
Views/                   メインウィンドウ、設定、全タブ検索、資格情報・拒否理由ダイアログ
installer/               WiX による MSI
```

### 実装上、後から触るときに注意が要る箇所

- **ウィンドウ位置は物理ピクセルで計算している**（`Services/WindowPlacer.cs`）。
  WPF の `Window.Left/Top` は混在 DPI のマルチモニタで素直に扱えないため、`SetWindowPos` を直接使う。
  設定に持つのは「サイズ = DIP」「位置 = 物理ピクセル」。
- **WinForms の暗黙 using を切っている**（`jp-scratch.csproj`）。
  通知領域アイコンのためだけに WinForms を参照しており、有効なままだと `Brush` `Point` `KeyEventArgs` などが WPF 側と全面衝突する。
- **IME 変換中は自動非表示を止める**（`NativeMethods.HasImeComposition`）。
  変換中にウィンドウが消えると入力そのものが失われる。メッセージのフックではなく、必要な瞬間に `ImmGetCompositionString` で問い合わせている。
- **テーマ辞書は `ThemeService` だけが差し込む**。App.xaml で読み込むと二重にマージされて切り替えが効かなくなる。
- **二重起動時の呼び戻しは名前付きイベントで行う**（`Infrastructure/SingleInstance.cs`）。
  ウィンドウメッセージのブロードキャストは使えない。`ShowInTaskbar="False"` のせいで WPF が隠しオーナーウィンドウを作るため
  メインウィンドウが「所有されたウィンドウ」になり、`PostMessage(HWND_BROADCAST)` の配送対象から外れる。
- **PowerShell スクリプトは UTF-8 BOM 付きで保存すること**。
  BOM がないと Windows PowerShell 5.1 が日本語を CP932 として読み、コメントが次の行を巻き込んで壊す。
